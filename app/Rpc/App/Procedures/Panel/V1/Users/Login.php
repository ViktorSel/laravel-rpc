...
class Login extends Procedure
{
    ...
    {
        if (Auth::attempt( array_merge($request->getParams(),[User::FIELD_ACTIVE => true]),true)) {
            $user = User::query()
                ->where(User::FIELD_EMAIL,'=',$request->input(User::FIELD_EMAIL))
                ->first();
